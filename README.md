...
- **Hear & read intervals** — each resolved interval shows its cents and, for just-intonation tunings, its ratio (3/2, 5/4, 7/4…); tap an interval to audition that degree briefly. Tunings that carry a suggested voicing offer an explicit **APPLY VOICING** button (never auto-applied on a tuning pick).
...
**Ableton Link** — the breathing LFO RATE syncs to Link tempo via a small chip (FREE / 8/1 / 4/1 / 2/1 / 1/1 / 1/2 / 1/4 / 1/8 / 1/16; the bar-multiple modes give breath cycles spanning several bars). When a synced mode is active and Link is connected, the breath is **phase-locked** so its peak lands on the bar's downbeat — alignment is tight on the slow/bar-spanning modes and looser on 1/8–1/16, where the bridge's latency is a larger share of a short cycle. Optionally, **Quantize changes to Link grid** (Settings → tempo; off by default — 1 beat / 1 bar / 2 bars) defers root, chord, preset, and sync-mode changes — and the drone's start — to the next grid boundary so they land in time with the other instruments; it quantizes changes only, not full transport. mdrone reuses mpump's [Link Bridge](https://github.com/gdamdam/mpump/releases) — a tiny cross-platform companion that bridges Link (UDP multicast) ↔ browser (localhost WebSocket). Run the bridge, enable Link in Settings, and any Link-enabled app syncs automatically. Nothing leaves your machine.

**mbus publish** — Settings → the **BUS** button (next to Link) offers the drone's master output to the [mbus](https://mbus.mpump.live) patchbay as a source named `mdrone`: tab-to-tab WebRTC signaled through the same Link Bridge, peer-to-peer, no server. Off by default, never persisted, harmless without the bridge. The vendored mbus-client lives in `src/transport/mbus/` (see its header for provenance).
