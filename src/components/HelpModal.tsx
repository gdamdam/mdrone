  ...
  return (
    <>
      ...
      <ul className="help-list">
        ...
        <li><strong>◉ MEDITATE</strong> — toggles a live visualizer tile above the WEATHER pad. Tap the canvas to expand to fullscreen.</li>
        <li><strong>▤ MIXER</strong> — opens the master bus drawer.</li>
        <li><strong>⤓ EXPORT AUDIO</strong> — header dropdown: REC LIVE / BOUNCE LOOP / TIMED REC.</li>
        <li><strong>◆ SESSION</strong> — header sheet: save / load / rename / EXPORT JSON / IMPORT JSON.</li>
      </ul>
...
function ReferenceTab() {
  return (
    <>
      ...
      <ul className="help-list">
        <li><strong>DRONE</strong> — the instrument. Three tiers: <em>Performance</em> (always visible — preset / HOLD / tonic / WeatherPad / macros), <em>EDIT</em> (one tap — INSTRUMENTS tiles + per-voice levels + FX BAR; each active tile shows a cream halo gauge for level, a thin RMS meter strip across the bottom, and hover-reveal <strong>M / S</strong> chips for mute and solo. Effect tiles mirror the halo for AMOUNT and surface a small dot when AMOUNT diverges from the engine default), <em>ADVANCED</em> (one tap — tuning, LFO, fine-detune, entrain).</li>
        ...
        <li><strong>MIXER</strong> — master bus: HPF, 3-band EQ, <strong>MUD</strong> trim toggle (-3.5 dB @ 300 Hz, on by default), glue, drive, look-ahead brickwall limiter + ceiling (-12 .. 0 dB), <strong>WIDTH</strong> (always-on bass-mono fold under 120 Hz), <strong>ROOM</strong> (parallel cathedral-IR send), <strong>COLOR</strong> (parallel saturation + air-band exciter). <strong>SAFE</strong> clamps the master to ~50% for headphones; when on, the header VOL pill shows a <strong>· HP</strong> badge so the cap is visible without opening the mixer.</li>
      </ul>
      ...
      <ul className="help-list">
        ...
        <li><strong>ATTUNE</strong> — guided randomize of the tuning layer only. Preserves preset voicing. Instantly beautiful microtonal state. Disabled during the 10 s engine-warming window after HOLD plus a 1.2 s click cooldown after each fire, so retunes can't stack a fresh voice rebuild on top of voices still settling.</li>
      ...
      <ul className="help-list">
        <li><strong>LINK</strong> — header button. Copies a self-contained URL of the current scene to your clipboard. Auto-shortened via the <code>s.mdrone.org</code> relay; if the relay is offline you get the full self-contained URL instead. Open it on any device and you land in the same drone landscape. Doubles as a personal bookmark — paste your own link anywhere to come back to a scene later. The Share modal warns when a URL exceeds ~1900 characters (some platforms truncate around 2000).</li>
        <li><strong>⤓ EXPORT AUDIO</strong> — header dropdown with three capture workflows. All three produce 24-bit stereo WAV via a parallel AudioWorklet tap (bit-identical, no codec), auto-start HOLD if the drone isn't playing, and prompt before close/reload while a take is in flight. Filenames carry scene name + tonic + preset, e.g. <code>mdrone-tidal-tape-a2-2026-04-29-1422.wav</code>:
          <ul className="help-list-compact">
            <li><strong>● REC LIVE</strong> — open-ended take. <em>WAV saved — M:SS</em> on stop. Long takes get a memory nudge; turn on <strong>Split {RECOMMENDED_MAX_TAKE_MINUTES} min</strong> to auto-save the take as bounded-memory <code>…-pt01.wav</code> / <code>…-pt02.wav</code> parts instead of one growing file.</li>
            <li><strong>◌ BOUNCE LOOP</strong> — sampler-ready short seamless loop (15 / 30 / 60 s) with a linear crossfade at the seam and a RIFF <code>smpl</code> chunk so samplers auto-detect the loop region. Filename includes the loop length: <code>…-loop-30s-…</code>.</li>
            <li><strong>⤓ TIMED REC</strong> — fixed-duration realtime capture (30 s · 1 m · 3 m · 10 m). Auto-stops and downloads. Realtime, not offline render. Filename includes the take length: <code>…-take-1m-…</code>.</li>
          </ul>
        </li>
        <li><strong>◆ SESSION</strong> — header sheet. <strong>SAVE</strong> / <strong>LOAD</strong> / <strong>RENAME</strong> a named local session, plus <strong>EXPORT JSON</strong> / <strong>IMPORT JSON</strong> for portable scene files (same payload as a share URL). All local-only, never uploaded.</li>
        <li><strong>REC MOTION</strong> (opt-in in Settings → GENERAL) — captures live gestures (60 s / 200 events) into the next share URL. <em>Not an audio file</em> — recipients hear the sweep you made by replaying the gestures over the synth.</li>
      ...
      <ul className="help-list">
        <li><strong>Low-Power Mode</strong> — Settings → GENERAL → LOW-POWER MODE (off by default). For older laptops, low-end Windows machines, and weak tablets. Clamps the MEDITATE visualizer to 15 fps, throttles the loudness meter, and skips a small preset-change duck. The MEDITATE rAF loop also adapts automatically — if frames drop, it steps down 30 → 20 → 15 → 10 fps without you doing anything.</li>
